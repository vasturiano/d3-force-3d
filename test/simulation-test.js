...
tape("forceSimulation() returns a simulation", function(test) {
  ...
  test.deepEqual(Object.keys(f).sort(), [ 'alpha', 'alphaDecay', 'alphaMin', 'alphaTarget', 'find', 'force', 'nodes', 'numDimensions', 'on', 'randomSource', 'restart', 'stop', 'tick', 'velocityDecay' ]);
  test.end();
});
